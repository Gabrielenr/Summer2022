...
def connect_to_mars():
    ...
    conn = pymysql.connect(host="mars.cs.qc.cuny.edu", port=3306, user="saya1406", passwd=password, database="saya1406")
    # cursor = conn.cursor()
...
def insert_state_data(states):
    ...
    cursor = conn.cursor()
    for state in states:
        query = "insert into state values ('" + state[0] + "','" + state[1] + "','" + state[2] + "')"
        print(query)
